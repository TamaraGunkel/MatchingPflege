...
def create_inquiry(db: Session, inquiry: schemas.InquiryCreate, customer_id: int, services: List[models.Service], times: List[models.InquiryTime]):
    db_inquiry = models.Inquiry(
        address_street=inquiry.address.street,
        address_number=inquiry.address.number,
        address_postal_code=inquiry.address.postal_code,
        address_city=inquiry.address.city,
        address_district=inquiry.address.district,
        level_of_care=inquiry.level_of_care,
        duration_in_minutes=inquiry.duration.total_seconds() / 60,
        hiring_start=inquiry.duration_hiring.start,
        hiring_end=inquiry.duration_hiring.end,
...
def get_inquiries(db: Session, skip: int = 0, limit: int = 100, district: str = None, status: str=None):
    result = (db.query(models.Inquiry)
            # filter for user selected to be contacted
            .filter(models.Inquiry.contact_opt_in))
            
    # filter for district
    if district:
        result = result.filter(models.Inquiry.address_district == district)
            
    # filter for status
    #.filter(status is None or models.Inquiry.status == status)
